...
from ifc_console_agents.chat.agent import converse, run_tool
...
async def collect(core, monkeypatch, script, **kwargs):
    monkeypatch.setattr("ifc_console_agents.chat.agent.stream", script)
    return [
...
async def test_tool_results_are_clipped(chat_core, monkeypatch):
    monkeypatch.setattr("ifc_console_agents.chat.agent.TOOL_RESULT_LIMIT", 200)
...
async def test_stopping_a_generation_releases_the_provider(chat_core, monkeypatch):
    ...
    monkeypatch.setattr("ifc_console_agents.chat.agent.stream", endless)
...
class TestToolEvent:
    ...
    def test_image_bytes_never_reach_the_transcript(self) -> None:
        from ifc_console_agents.chat.agent import TOOL_PREVIEW_LIMIT, tool_event

...
async def test_keys_can_be_saved_and_deleted_without_ever_being_returned(
    ...
    saved: dict[str, str] = {}
    monkeypatch.setattr("ifc_console_agents.credentials.keyring_available", lambda: True)
    monkeypatch.setattr("ifc_console_agents.credentials.stored_providers", lambda _home: list(saved))
    monkeypatch.setattr(
        "ifc_console_agents.credentials.set_api_key",
        lambda _home, provider, key: saved.__setitem__(provider, key),
    )
    monkeypatch.setattr(
        "ifc_console_agents.credentials.delete_api_key",
...
async def test_local_only_blocks_model_discovery_before_network(chat_core, monkeypatch):
    ...
    monkeypatch.setattr("ifc_console_agents.chat.routes.list_models", should_not_run)
    client = _client(chat_core)
    response = client.post(
        "/api/chat/models",
        headers=_auth(chat_core),
...
async def test_model_discovery_returns_capabilities_without_breaking_the_id_list(
    ...
    from ifc_console_agents.chat.providers import ModelList
    ...
    monkeypatch.setattr("ifc_console_agents.chat.routes.list_models", lambda *_args, **_kwargs: discovered)
    client = _client(chat_core)
...
async def test_a_provider_failure_arrives_as_an_event_not_a_500(chat_core, monkeypatch):
    from ifc_console_agents.chat.providers import ProviderError

    def failing(provider, **kwargs):
        raise ProviderError("HTTP 401: invalid api key")
        yield  # pragma: no cover

    monkeypatch.setattr("ifc_console_agents.chat.agent.stream", failing)
    client = _client(chat_core)
    response = client.post(
        "/api/chat/stream",
        headers=_auth(chat_core),
...
async def test_the_chat_page_and_assets_ship_with_the_agents_extension(chat_core):
    from ifc_console_agents import assets

    directory = assets.require_static_dir()
    for name in ("chat.html", "chat.js", "chat.css"):
        assert (directory / name).is_file(), f"{name} missing from the agents bundle"
    client = _client(chat_core)
    assert client.get("/agents/static/chat.js").status_code == 200

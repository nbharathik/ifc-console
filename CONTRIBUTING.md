...
- **Setup, tests, style, release process**: see the
  ...
  `uv run ruff check src tests`; all three must be green.
...
- **Compatibility**: the MCP tool names, input schemas, the response envelope,
  and the `ifc_console.sdk` surface are the public API. Changes to them need a
  changelog entry and, pre-1.0, a minor version bump. Run
